								...
								<tbody>
									@forelse ($items as $item)
										@php
											...
											$image = !empty($product->productImages->first()) ? asset(env('BE_APP_URL').'storage/'.$product->productImages->first()->path) : asset('themes/ezone/assets/img/cart/3.jpg')
										@endphp
										<tr>
											<td class="product-remove">
                                                <a href="{{ url('/carts/delete/'. $item->id)}}" class="delete btn-delete"><i class="pe-7s-close"></i></a>
											</td>
											...
											<td class="product-quantity">
                                                ...
                                                <input name="items[{{$item->id}}][quantity]" type="number" class="form-control" value="{{$item->quantity}}" min="1" required>
											</td>
