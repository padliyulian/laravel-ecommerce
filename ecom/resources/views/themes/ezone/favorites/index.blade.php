								...
								<tbody>
									@forelse ($favorites as $favorite)
										@php
											...
											$image = !empty($product->productImages->first()) ? asset(env('BE_APP_URL').'storage/'.$product->productImages->first()->small) : asset('themes/ezone/assets/img/cart/3.jpg')
										@endphp
										<tr>
											<td class="product-remove">
												{!! Form::open(['url' => 'favorites/'. $favorite->id, 'class' => 'delete', 'style' => 'display:inline-block']) !!}
                                                {!! Form::hidden('_method', 'DELETE') !!}
                                                <button type="submit" style="background-color: transparent; border-color: #FFF;" class="js-btn--delete">X</button>
                                                {!! Form::close() !!}
											</td>
