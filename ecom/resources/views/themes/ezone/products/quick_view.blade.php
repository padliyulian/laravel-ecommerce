					...
					<div class="quick-view-tab-content tab-content">
						...
						@foreach ($product->productImages as $image)
							<div class="tab-pane fade {{ ($i == 1) ? 'active show' : '' }}" id="modal{{ $i}}" role="tabpanel">
								@if ($image->medium)
									<img src="{{ asset(env('BE_APP_URL').'storage/'.$image->medium) }}" alt="{{ $product->name }}" style="width:320px;">
								@else
				...
				<div class="quick-view-list nav" role="tablist">
					...
					@foreach ($product->productImages as $image)
						<a class="{{ ($i == 1) ? 'active' : '' }} mr-12" href="#modal{{ $i }}" data-toggle="tab" role="tab">
							@if ($image->small)
								<img src="{{ asset(env('BE_APP_URL').'storage/'.$image->small) }}" alt="{{ $product->name }}" style="width:100px; height:112px">
							@else
