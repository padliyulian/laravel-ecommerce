									...
									<div class="tab-pane fade {{ ($i == 1) ? 'active show' : '' }}" id="pro-details{{ $i}}" role="tabpanel">
										<div class="easyzoom easyzoom--overlay">
											@if ($image->large && $image->extra_large)
												<a href="{{ asset(env('BE_APP_URL').'storage/'.$image->extra_large) }}">
													<img src="{{ asset(env('BE_APP_URL').'storage/'.$image->large) }}" alt="{{ $product->name }}">
												</a>
											@else
							...
							<div class="product-details-small nav mt-12" role=tablist>
								...
								@forelse ($product->productImages as $image)
									<a class="{{ ($i == 1) ? 'active' : '' }} mr-12" href="#pro-details{{ $i }}" data-toggle="tab" role="tab" aria-selected="true">
										@if ($image->small)
											<img src="{{ asset(env('BE_APP_URL').'storage/'.$image->small) }}" alt="{{ $product->name }}">
										@else
